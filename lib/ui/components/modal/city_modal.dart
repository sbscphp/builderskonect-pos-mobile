...
class CityModal extends ConsumerStatefulWidget {
  const CityModal({super.key, required this.stateId});
...
class _CustomListModalState extends ConsumerState<CityModal> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await ref.read(geographyVmodel).getCities(widget.stateId);
    });
  }
  ...
  void _searchCity(String query) {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      ref.read(geographyVmodel).getCities(widget.stateId, query);
    });
  }
              ...
              contentBuilder: (context) {
                return RefreshIndicator(
                  ...
                  child: ListView.separated(
                    ...
                    shrinkWrap: true,
                    itemCount: geographyVm.cities.length,
                    separatorBuilder: (_, __) => YBox(24),
                    itemBuilder: (_, i) {
                      final item = geographyVm.cities[i];
                      return InkWell(
                        onTap: () {
